 ...
 *
 */
public class LearnedWordsNotSyncedProvider extends ContentProvider {
	
	// defining ContentProvider's URI address
	public static final String AUTHORITY = "pl.elector.provider.LearnedWordsNotSyncedProvider"; 
	private static final String BASE_PATH = "learned_words_not_synced"; 
	...
	// 4) single row for specific learned id 
	...
	// in 'learned_words_not_synced' represents a request for all items
	// and 'learned_words_not_synced/[rowId]' represents a single row,
	// and 'learned_words_not_synced/profile/[profileId]/word/[wordId]' represents a single row for given profile and word id
	// and 'learned_words_not_synced/profile/[profileId]' represents request for subset of rows for given profile id,
	static {
	...
	// and a single row. 
	private static final String CONTENT_MIME_TYPE = ContentResolver.CURSOR_DIR_BASE_TYPE + "/vnd.elector.learned_words_not_synced"; 
	private static final String CONTENT_ITEM_MIME_TYPE = ContentResolver.CURSOR_ITEM_BASE_TYPE + "/vnd.elector.learned_words_not_synced"; 
	...
	public synchronized int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		// Limit the deletion to specified row or rows in Content URI
		switch( uriMatcher.match(uri))
		{
			case SINGLE_ROW: {
				String rowID = uri.getPathSegments().get(1); 
				selection = LearnedWordsNotSyncedTable.COLUMN_LEARNED_WORDS_NOT_SYNCED_ID + "=" + rowID
			...
			case ROW_FOR_PROFILE_AND_WORD: {
				...
				selection = LearnedWordsNotSyncedTable.COLUMN_PROFILE_ID + "=" + profileID
						+ " AND " + LearnedWordsNotSyncedTable.COLUMN_WORD_ID + "=" + wordID
						+ (!TextUtils.isEmpty(selection) ? " AND ("
						+ selection + ")" : "");
				break; 
			}
		...
		int deleteCount = db.delete(LearnedWordsNotSyncedTable.TABLE_LEARNED_WORDS_NOT_SYNCED, selection, selectionArgs);
	...
	public synchronized Uri insert(Uri uri, ContentValues values) {
		...
		switch( uriMatcher.match(uri) ) 
		{
			case ALLROWS: 
				...
				id = db.insert(LearnedWordsNotSyncedTable.TABLE_LEARNED_WORDS_NOT_SYNCED, nullColumnHack, values); 
				break; 
			default: 
	...
	public synchronized Cursor query(Uri uri, String[] projection, String selection,
		...
		switch( uriMatcher.match(uri) )
		{
			case SINGLE_ROW: 
			{
				queryBuilder.setTables(LearnedWordsNotSyncedTable.TABLE_LEARNED_WORDS_NOT_SYNCED);
				// getting rowID for current query and setting where clause 
				String rowID = uri.getPathSegments().get(1); 
				queryBuilder.appendWhere(LearnedWordsNotSyncedTable.COLUMN_LEARNED_WORDS_NOT_SYNCED_ID + "=" + rowID);
				break; 
			}
			case ROW_FOR_PROFILE_AND_WORD: 
			{
				...
				String profileID = uri.getPathSegments().get(2); 
				String wordID = uri.getPathSegments().get(4); 
				queryBuilder.appendWhere(LearnedWordsNotSyncedTable.COLUMN_PROFILE_ID + "=" + profileID
									+ " AND " + LearnedWordsNotSyncedTable.COLUMN_WORD_ID + "=" + wordID);
				break; 
			}
			case ROWS_FOR_PROFILE: 
			...
			{
				String profileID = uri.getPathSegments().get(2); 
				String selectionSQL = "SELECT * FROM " + LearnedWordsNotSyncedTable.TABLE_LEARNED_WORDS_NOT_SYNCED
									  + " AS NS "
									  + " WHERE NS." + LearnedWordsNotSyncedTable.COLUMN_PROFILE_ID + "= ? "
									  + " AND NS." +LearnedWordsNotSyncedTable.COLUMN_TO_DELETE + " = 0"
									  + " AND (SELECT LR." + LearnedWordsProvider.LearnedTable.COLUMN_LEARNED_ID
							          + " FROM " + LearnedWordsProvider.LearnedTable.TABLE_LEARNED
									  + " AS LR "
									  + " WHERE LR." + LearnedWordsProvider.LearnedTable.COLUMN_PROFILE_ID + " = ? "
									  + " AND LR." + LearnedWordsProvider.LearnedTable.COLUMN_WORD_ID  
									  + " = NS." + LearnedWordsNotSyncedTable.COLUMN_WORD_ID + " LIMIT 1) IS NULL;";
				String[] whereArgs = new String[] { profileID, profileID, };
				
				Log.w(LearnedWordsNotSyncedProvider.class.getName(), "Executing query: " + selectionSQL); 
	...
	public synchronized int update(Uri uri, ContentValues values, String selection,
		...
		// Modify selection argument to indicate updated row or rows in Content URI.
		switch( uriMatcher.match(uri))
		{
			case SINGLE_ROW: {
				String rowID = uri.getPathSegments().get(1); 
		...
		// Perform the update.
		int updateCount = db.update(LearnedWordsNotSyncedTable.TABLE_LEARNED_WORDS_NOT_SYNCED, 
	...
	public static class LearnedWordsNotSyncedTable { 
		
		// Database Table 
		public static final String TABLE_LEARNED_WORDS_NOT_SYNCED = "learnedWordsNotSyncedTable"; 
		public static final String COLUMN_LEARNED_WORDS_NOT_SYNCED_ID = "_id"; // primary key
		...
		private static final String TABLE_CREATE = "create table if not exists "
				+ TABLE_LEARNED_WORDS_NOT_SYNCED
				+ " ("
				+ COLUMN_LEARNED_WORDS_NOT_SYNCED_ID + " integer primary key autoincrement, "
		...
		// TRIGGERS: 
		// 1) insert trigger on learned_words_not_synced table, checks if corresponding profile exists
		private static final String PROFILE_INSERT_TRIGGER_CREATE = "create trigger fki_"
				+ TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_PROFILE_ID + " "
				+ "before insert on " + TABLE_LEARNED_WORDS_NOT_SYNCED + " "
				+ "for each row begin "
					+ "select raise(rollback, 'insert on table " + TABLE_LEARNED_WORDS_NOT_SYNCED
		...
		// 2) insert trigger on learned_words_not_synced table, checks if corresponding word exists
		private static final String WORD_INSERT_TRIGGER_CREATE = "create trigger fki_"
				+ TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_WORD_ID + " "
				+ "before insert on " + TABLE_LEARNED_WORDS_NOT_SYNCED + " "
				+ "for each row begin "
						+ "select raise(rollback, 'insert on table " + TABLE_LEARNED_WORDS_NOT_SYNCED
										+ " violates foreign key constraint') "
		...
		// 3) update trigger on learned_words_not_synced table, checks if new profile exists
		private static final String PROFILE_UPDATE_TRIGGER_CREATE = "create trigger fku_"
				+ TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_PROFILE_ID + " "
				+ "before update on " + TABLE_LEARNED_WORDS_NOT_SYNCED + " "
				+ "for each row begin "
						+ "select raise(rollback, 'update on table " + TABLE_LEARNED_WORDS_NOT_SYNCED
										+ " violates foreign key constraint') "
		...
		// 4) update trigger on learned_words_not_synced table, checks if new word exists
		...
		// 5) delete trigger on profile table, cascade deletes corresponding learned words not_synced
		private static final String PROFILE_DELETE_TRIGGER_CREATE = "create trigger fkd_"
				...
				+ "for each row begin "
					+ "delete from " + TABLE_LEARNED_WORDS_NOT_SYNCED
							+ " where " + COLUMN_PROFILE_ID
							+ " = old." + ProfileProvider.ProfileTable.COLUMN_PROFILE_ID + ";"
				+ " end;"; 
		
		// 6) delete trigger on word table, cascade deletes corresponding learned words not_synced
		...
		// 7) update trigger on profile table, cascade updates corresponding learned words not_synced
		private static final String PROFILE_PARENT_UPDATE_TRIGGER_CREATE = "create trigger fkpu_"
				...
				+ "for each row begin "
						+ "update " + TABLE_LEARNED_WORDS_NOT_SYNCED + " set " + COLUMN_PROFILE_ID 
		...
		// 8) update trigger on word table, cascade updates corresponding learned words not_synced
		private static final String WORD_PARENT_UPDATE_TRIGGER_CREATE = "create trigger fkpu_"
				...
				+ "for each row begin "
					+ "update " + TABLE_LEARNED_WORDS_NOT_SYNCED + " set " + COLUMN_WORD_ID 
		...
		{
			// LearnedWordsNotSynced table creation in database (with additional triggers)
		...
		public static void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion)
		{
			...
			Log.w(LearnedWordsNotSyncedTable.class.getName(), 
					"Upgrading database LearnedWordsNotSynced table from version " + oldVersion 
			...
			// Upgrade database by adding new version of LearnedWordsNotSynced table?
			database.execSQL("DROP TABLE IF EXISTS " + TABLE_LEARNED_WORDS_NOT_SYNCED); 
			database.execSQL("DROP TRIGGER IF EXISTS fki_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_PROFILE_ID);
			//database.execSQL("DROP TRIGGER IF EXISTS fki_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_WORD_ID);
			database.execSQL("DROP TRIGGER IF EXISTS fku_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_PROFILE_ID); 
			//database.execSQL("DROP TRIGGER IF EXISTS fku_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_WORD_ID); 
			database.execSQL("DROP TRIGGER IF EXISTS fkd_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_"+ COLUMN_PROFILE_ID); 
			//database.execSQL("DROP TRIGGER IF EXISTS fkd_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_WORD_ID); 
			database.execSQL("DROP TRIGGER IF EXISTS fkpu_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_PROFILE_ID);
			//database.execSQL("DROP TRIGGER IF EXISTS fkpu_" + TABLE_LEARNED_WORDS_NOT_SYNCED + "_" + COLUMN_WORD_ID);
			onCreate(database); 
		}
	}
