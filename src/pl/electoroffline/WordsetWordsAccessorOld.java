 ...
 *
 */
public class WordsetWordsAccessorOld implements Personalization.Callbacks {
    ...
    private LinkedHashMap<Integer, Bitmap> imageBitmaps; 
    ...
    private WordsetWordsAccessorOld.Callbacks listener;
    
    public interface Callbacks {
    	
    	public void onWordsLoadFinished(WordsetWordsAccessorOld accessor); 
    }
	 ...
	 */
	public WordsetWordsAccessorOld(Context ctx, int wordsetId) {
	 ...
	 */
	public WordsetWordsAccessorOld(Context ctx, int wordsetId, boolean wordsetPersonalize)
	{
		this(ctx, wordsetId, WordsetType.SYSTEM_WORDSET, wordsetPersonalize); 
	}
	
	public WordsetWordsAccessorOld(Context ctx, int wordsetId, WordsetType type) {
	 ...
	 */
	public WordsetWordsAccessorOld(Context ctx, int wordsetId, WordsetType type, boolean wordsetPersonalize) {
			this.context = ctx;
			if(ctx instanceof WordsetWordsAccessorOld.Callbacks) 
				this.listener = (WordsetWordsAccessorOld.Callbacks) ctx; 
			...
			this.url = context.getResources().getString(R.string.getwordset_url) + wordsetId + wordsetType;
			Log.w(WordsetWordsAccessorOld.class.getName(), "Wordset URL: " + this.url);
	}
	 ...
	 * @param l
	 */
	public void setCallbacksListener(WordsetWordsAccessorOld.Callbacks l) 
	{
		this.listener = l; 
	}
	...
	{	
		...
		((Activity) context).runOnUiThread(new Runnable() {
			...
			public void run() {
				if(listener != null)
					listener.onWordsLoadFinished(WordsetWordsAccessorOld.this); 
			} 
	...
	public LinkedHashMap<Integer, Bitmap> getImageBitmaps()
	{
	...
	private void getWordsetWords() 
	{
		...
		if(Preferences.getBoolean(context, Preferences.KEY_PREFER_ONLINE_DATA, false)) 
		{
	...
	{
		Log.w(WordsetWordsAccessorOld.class.getName(), "Loading wordset words from web service...");
		...
		try { 
			...
			Log.w(WordsetWordsAccessorOld.class.getName(), "Loading words from URL: "+ url); 
			...
			Log.w(WordsetWordsAccessorOld.class.getName(), "Number of loaded words from online: " + wordsListObject.getENWords().size());
			...
			Log.w(WordsetWordsAccessorOld.class.getName(), "Number of word Ids: " + foreignWords.keySet().size());
	    
			// wordset words has been loaded from online web service 
			Log.w(WordsetWordsAccessorOld.class.getName(), "Wordset words has been loaded successfully from online web service.");
			areWordsLoaded = true; 
			notifyAll(); 
			
		} catch (Exception e) {
			e.printStackTrace();
			Log.w(WordsetWordsAccessorOld.class.getName(), "Wordset words hasn't been loaded from online web service!");
	...
	{
		Log.w(WordsetWordsAccessorOld.class.getName(), "Loading wordset words from local storage...");
	...
	private void loadNotSystemWordsetWordsFromLocalStorage() {
		...
		Log.w(WordsetWordsAccessorOld.class.getName(), "Decided to load not system wordset words from local "
		...
		Log.w(WordsetWordsAccessorOld.class.getName(), "Retrieved " + cursor.getCount() + " number of not system words."); 
		...
		String recordingPath = WordsetWordsAccessorOld.CHECK_RECORDING_AVAILABILITY; 
		
		fillWordDetailsFromCursor(cursor, recordingPath, false); // recording name not prepended with id
	}
	
	private void fillWordDetailsFromCursor(Cursor cursor, String recordingPath, boolean prependRecordingNameWithWordId)
	{
		Log.w(WordsetWordsAccessorOld.class.getName(), "Filling word details from cursor with: " + cursor.getCount() + " number of words.");
		...
		recordingPaths = new LinkedHashMap<Integer, String>(); 
		imageBitmaps = new LinkedHashMap<Integer, Bitmap>(); 
		...
		while(cursor.moveToNext())
		{
			...
			if(prependRecordingNameWithWordId) 
				recordingPaths.put(wordId, recordingPath + "/"   
						+ wordId + cursor.getString(cursor.getColumnIndexOrThrow(WordProvider.WordTable.COLUMN_RECORDING)));
			else
				recordingPaths.put(wordId, recordingPath + "/" 
											+ cursor.getString(cursor.getColumnIndexOrThrow(WordProvider.WordTable.COLUMN_RECORDING)));
			...
			if(image != null)
				imageBitmaps.put(wordId, BitmapFactory.decodeByteArray(image, 0, image.length));
			else 
				imageBitmaps.put(wordId, null);
	...
	private void loadWordsFromLocalStorageAndAudioOnline() 
	{
		...
		Log.w(WordsetWordsAccessorOld.class.getName(), "Deciding to load wordset words from loacal" +
	...
	private boolean isAudioStoredLocally()
	{
		...
		if(cursor.moveToFirst()) {
			...
			Log.w(WordsetWordsAccessorOld.class.getName(), "Wordset not found while checking is audio stored locally!");
		}
		cursor.close(); 
		return false; 
	}
	...
	private boolean areWordsAvailableInDatabase(int wordsetId, WordsetType type)
	{
		...
		if(countWordsSavedInDatabase(wordsetId) > 0) { 
				Log.w(WordsetWordsAccessorOld.class.getName(), "There are some words saved in DB for wordset."); 
				return true; 
		}
		Log.w(WordsetWordsAccessorOld.class.getName(), "There aren't any words saved in DB for wordset."); 
		return false; 	
	}
	...
	private boolean areWordsAvailableInDatabase(WordsetType type)
	{
		switch(type) { 
			...
			case SYSTEM_WORDSET: 
			default: 
				Log.w(WordsetWordsAccessorOld.class.getName(), "ERROR: wrong wordset type for wordsetId = 0.");
				return false; 
		}
	}
	...
	private boolean areSelectedWordsAvailableInDatabase() 
	{
		...
		if(selectedWordIds != null && selectedWordIds.size() > 0) { 
			...
			{
				Log.w(WordsetWordsAccessorOld.class.getName(), 
	...
	private int countWordsSavedInDatabaseFor(ArrayList<String> wordIds)
	{
		...
		String[] selectionArgs =  wordIds.toArray(new String[] {});  // is it ok ? 
 		...
 		if(cursor.moveToFirst()) 
 		{
 				...
 				cursor.close(); 
 				Log.w(WordsetWordsAccessorOld.class.getName(), "Words count for selected word ids is: " + count); 
 				return count; 
 		}
 		cursor.close(); 
 		return 0; 	 
	...
	private int countWordsSavedInDatabase(int wordsetId) 
	{
		...
		if(cursor.moveToFirst()) 
		{
			...
			int count = cursor.getInt(0); 
			cursor.close(); 
			Log.w(WordsetWordsAccessorOld.class.getName(), "Words count in wordset is: " + count); 
			return count; 
		}
		cursor.close(); 
		return 0; 	 
...
(		(Activity) context).runOnUiThread( new Runnable() {
									...
									.setPositiveButton(R.string.connect_button,  new DialogInterface.OnClickListener() {
										...
										public void onClick(DialogInterface dialog, int which) {
											...
											if(connected) {
												...
												Log.w(WordsetWordsAccessorOld.class.getName(), "Trying to connect to the Internet failed! Emergency Mode.");
	...
	private synchronized void loadWordsFromXMLResources() 
	{
        ...
        Log.w(WordsetWordsAccessorOld.class.getName(), "Wordset words has been loaded successfully from raw XML.");
        areWordsLoaded = true; 
        notifyAll(); 
	...
	private void synchronizeSystemWordset() 
	{
		...
		Intent syncIntent = new Intent(context, WordsetActivityOld.class); 
		syncIntent.putExtra(WordsetsListActivity.SELECTED_WORDSET, wordsetId); 
		syncIntent.putExtra(WordsetActivityOld.START_WORDS_SYNCHRONIZATION, true); 
	...
	{
		Log.w(WordsetWordsAccessorOld.class.getName(), selectedWordIds.toString());
		...
		syncIntent.putStringArrayListExtra(WordsetWordsAccessorOld.KEY_WORDS_TO_SYNC, selectedWordIds); 
		syncIntent.putExtra(WordsetWordsAccessorOld.KEY_START_SELECTED_WORDS_SYNCHRONIZATION, true);
		
		Log.w(WordsetWordsAccessorOld.class.getName(), "Starting selected words synchronization..."); 
